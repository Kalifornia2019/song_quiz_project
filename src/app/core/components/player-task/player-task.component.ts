...
export class PlayerTaskComponent implements OnInit,OnChanges {
  ...
  play(){
    ...
    this.audioPlayerRef.nativeElement.addEventListener("timeupdate", () => this.updatePlayer());

  }

  updatePlayer() {
    const currentTime = Math.floor(this.audioPlayerRef.nativeElement.currentTime);
    ...
    const duration = Math.floor(this.audioPlayerRef.nativeElement.duration) || 30;
